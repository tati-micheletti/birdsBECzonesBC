...
#    `functionName` is a function's name in your module (e.g., `prepingInputsEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/tmichele/GitHub/birdsBECzonesBC/modules/prepingInputs/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("prepingInputs")
  ...
  parameters <- list(
    ...
    prepingInputs = list(.saveInitialTime = NA)
  )
  ...
  if (exists("prepingInputsEvent1", envir = .GlobalEnv)) {
    simOutput <- prepingInputsEvent1(mySim)
  } else {
  ...
  if (exists("prepingInputsEvent2", envir = .GlobalEnv)) {
    simOutput <- prepingInputsEvent2(mySim)
  } else {
