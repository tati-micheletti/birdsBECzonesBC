...
#    `functionName` is a function's name in your module (e.g., `birdPredictionBRTEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/tmichele/GitHub/birdsBECzonesBC/modules/birdPredictionBRT/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("birdPredictionBRT")
  path <- list(modulePath = "C:/Users/tmichele/GitHub/birdsBECzonesBC/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    birdPredictionBRT = list(.saveInitialTime = NA)
  )
  ...
  if (exists("birdPredictionBRTEvent1", envir = .GlobalEnv)) {
    simOutput <- birdPredictionBRTEvent1(mySim)
  } else {
  ...
  if (exists("birdPredictionBRTEvent2", envir = .GlobalEnv)) {
    simOutput <- birdPredictionBRTEvent2(mySim)
  } else {
